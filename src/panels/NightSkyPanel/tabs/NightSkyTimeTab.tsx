...
import { Stack, Group, Text, Space, Divider, Button, Paper } from '@mantine/core';
...
export function NightSkyTimeTab() {
    ...
    return (
        <>
                <Group mb={'md'}>
                    <Text my={'md'} size='xl'>Simulation Time</Text> 
                    <Button onClick={
                        async () => {
                            let now = await luaApi?.time.currentWallTime()
                            if (now) {
                                luaApi?.time.setTime(now);
                            } else {
                                let ceftime = new Date().toUTCString()
                                luaApi?.time.setTime(ceftime);
                            }
                        }}>Set to now
                    </Button>
                </Group>
                <Text size={'xl'} style={{ flexGrow: 12}}>Local: {localTimeString}</Text>
                <Text mb={'lg'} style={{ flexGrow: 12}}>Timezone: {localArea}</Text>
                <Text my={'md'} style={{ flexGrow: 12}}>UTC: {timeLabel}</Text>
                <Divider mt={5} />
                <Text my={'md'} size='xl'>Jumps</Text>
                <Stack mb={'lg'}>
                    <Group gap={"xl"}>
                        <Group>
                            <Button
                ...
                <Paper my={'lg'} shadow="xs" p="xl">
                    <Text>Only some controls are found here. For more control over time, use the Time Panel.</Text>
                </Paper>
        </>
    );
}