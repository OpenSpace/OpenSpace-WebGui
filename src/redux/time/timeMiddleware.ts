...
export const setupSubscription = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    ...
    function updateFunc(updates: Partial<TimeData>) {
    ...
    const batcher = new Batcher<TimeData>(updateFunc, batchTime);

    (async () => {
      for await (const data of timeTopic) {
        batcher.add(data);
      }
    })();
  }
